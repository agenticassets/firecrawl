...
**Agent Requested** (`alwaysApply: false` + `description`, **no `globs` field**) - AI autonomously decides whether to fetch the rule based on description. The agent first evaluates the description, then requests the full rule if relevant. If `globs` is present, the rule is Auto Attached regardless of `description`.[^10]
...
- **Mode behavior**: When `globs` is present, the rule is Auto Attached (regardless of `description`). When only `description` is present (no `globs`), the rule is Agent Requested. The `description` field can be included alongside `globs` for documentation purposes, but it doesn't change the Auto Attached behavior.
...
**Mode precedence**: If `globs` is present, the rule is Auto Attached (even if `description` is also present). Agent Requested mode only applies when `globs` is absent and `description` is present. The `description` field can coexist with `globs` for documentation but doesn't enable Agent Requested behavior.[^15]
